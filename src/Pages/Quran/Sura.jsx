    ...
    <div>
      {loading ? (
        ...
        <div className="sura">
          {data.map((item) => (
            <div key={item.number} className="relative py-4">
              <p className="text-lg md:text-xl leading-loose">
                {item.text} (<span>{item.numberInSurah}</span>)
              </p>
             ...
             {/* tafsir container  */}
                {tafsir[item.number] && (
                  <div className="mr-[30px] mt-3 bg-green-50 p-3 rounded-md text-green-700 relative">
                    <h6 className="text-center text-[16px] sm:text-[22px] pt-[27px]">{tafsir[item.number]}</h6>
                  {/* copy tafsir */}
          ...
          <Link to="/quran" className="inline-block mt-6 text-gray-600 hover:text-black">
