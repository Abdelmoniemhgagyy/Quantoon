  ...
  if (loading) {
    return (
      <div className="w-full h-[60vh] flex flex-col items-center justify-center">
        ...
        <div className="text-lg text-teal-400 font-bold">جاري تحميل الجزء...</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="w-full max-w-4xl relative">
        ...
        >
          <ArrowRight size={18} />
          <span className="hidden sm:inline font-medium text-sm">عودة للأجزاء</span>
        </Link>
        ...
        <div className="mt-12 sm:mt-10 space-y-12 text-center relative z-10 w-full">
          ...
          >
            <div className="px-8 py-2 rounded-full border border-teal-400/50 bg-teal-500/10 text-teal-300 font-bold text-xl shadow-sm">
              الجزء {id}
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            {data.map((item) => (
              <React.Fragment key={item.number}>
                ...
                >
                  {item.numberInSurah || item.number}
                </span>
